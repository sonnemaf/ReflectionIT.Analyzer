...
namespace RefactoringEssentials.Tests.CSharp.CodeRefactorings
{
    ...
    {
        public void Test<T>(string input, string output, int action = 0, CSharpParseOptions parseOptions = null)
            where T : CodeRefactoringProvider, new()
        {
            Test(new T(), input, output, action, parseOptions);
        }

        public void Test(CodeRefactoringProvider provider, string input, string output, int action = 0, CSharpParseOptions parseOptions = null)
        {
            string result = HomogenizeEol(RunContextAction(provider, HomogenizeEol(input), action, parseOptions));
        ...
        protected string RunContextAction(CodeRefactoringProvider action, string input, int actionIndex = 0, CSharpParseOptions parseOptions = null)
        {
            Document doc;
            CSharpDiagnosticTestBase.TestWorkspace workspace;
            ...
            if (a == null) {
                throw new ActionNotAvailableException("No action found");
            }
    ...
    [Serializable]
    public class ActionNotAvailableException : Exception {
        public ActionNotAvailableException() { }
        public ActionNotAvailableException(string message) : base(message) { }
        public ActionNotAvailableException(string message, Exception inner) : base(message, inner) { }
        protected ActionNotAvailableException(
